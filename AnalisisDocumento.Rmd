...
ggplot(data=tdata,aes(x=MONTHSBEFORE, y=PENDINGPAYS, fill=default))+geom_boxplot(outlier.shape = NA, alpha=0.6)+labs(title="Pagos pendientes cada mes", subtitle="vs Impago")+ylab("Pagos Pendientes")+xlab("Meses antes")+geom_text(data=fcount,aes(x=MONTHSBEFORE,label=cnt,y=7))
```
...
```{r}
ggplot(aes(x=FP,fill=default),data=fdata)+geom_bar(position = position_stack())+labs(title="Proporción de impago",subtitle = "Por categoría predicha")+xlab("Categoría")+ylab("Proporción")
```
