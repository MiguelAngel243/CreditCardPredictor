...
Sin embargo, el modelo producido por regresión regularizada parece recolectar mejor (aunque en forma muy sutil) toda la información presente en el dataset. De este modelo resalta una disminución en la varianza del valor de ROC, lo que podría estar relacionado con un menor sobreajuste, dado que la regresión regularizada mitiga los efectos de las variables correlacionadas y que por ende, provocan sobreajuste.

### Interpretación de Coeficientes del modelo 4

```{r}
coef <- as.data.frame(as.matrix(coef(model4$finalModel,model4$bestTune$lambda)))
colnames(coef) <- "Valor"
...
ggplot(aes(x=fct_reorder(rownames(coef),Valor),y=Valor),data=coef)+geom_point(color="midnightblue")+
  labs(title="Coeficientes",subtitle="Del modelo de regresión regularizada")+
...
La variable Meses Irregulares y Pagos pendientes el mes pasado (PAY_1) tienen un coeficiente alto, indicando relevancia en la predicción. Todas las variables numéricas no tienen interpretación directa con unidades monetarias, años, o pagos pendientes, ya que todas las variables fueron estandarizadas para aplicar el metodo de regularización.
...
dplyr::arrange(data.frame(model5$finalModel$importance),desc(MeanDecreaseGini))
```

Se enlistan en orden decreciente las variables dentro del modelo RandomForest por importancia según el criterio Gini. El más importante es la cantidad de pagos pendientes el mes pasado, seguido de dos variables derivadas y los registros
...
```{r }
gr <- expand.grid(C=c(0.01,0.03,0.05,0.06,0.1))
model6 <- train(default~.,data=fdata,method="svmLinear",tuneGrid=gr,metric="ROC",trControl=cntl,preProcess=c("center","scale"))

model7 <- lda(default~.,data=odata)

predMat <- matrix(c(
  model5$finalModel$votes[,2],
  #predict(model6,fdata,"prob")[,2],
  predict(model7,odata)$posterior[,2]
  ),30000,3)
